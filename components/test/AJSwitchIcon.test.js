...
describe('integration tests', () => {
  it("has altering 'not selected' classes when the switch is toggled", () => {
    ...
    expect(wrapper.find('.aj-switch-icon-on').hasClass(/(ajSwitchIconNotSelected)(-\d+)/)).toBe(false);
    expect(wrapper.find('.aj-switch-icon-off').hasClass(/(ajSwitchIconNotSelected)(-\d+)/)).toBe(true);

    wrapper.find('.aj-switch').simulate('click');

    expect(wrapper.find('.aj-switch-icon-on').hasClass(/(ajSwitchIconNotSelected)(-\d+)/)).toBe(true);
    expect(wrapper.find('.aj-switch-icon-off').hasClass(/(ajSwitchIconNotSelected)(-\d+)/)).toBe(false);
  });
